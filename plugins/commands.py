...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('⌬ Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url='https://t.me/namaste_cineverse')
        ],[
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url="https://t.me/namaste_cineverse_bot")
                    ],
                    # [
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                ...
                # [
                #     InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @Namaste_CineVerse_Bot
                # ]
                ]
            else:
                button = [[
    ...
    if not files_:
        ...
        try:
            ...
            title = '@namaste_cineverse ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@namaste_cineverse ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    #     except Exception as e:
    #         logger.exception(e)
    #         #TODO REMOVED THIS LINE f_caption=f_caption
    if f_caption is None:
        f_caption = f"@namaste_cineverse {file.file_name.replace('cineverse','')}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
