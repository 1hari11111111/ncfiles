...
# Ask Doubt on telegram @Namaste_CineVerse_Bot
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/namaste_cineverse>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/namaste_cineverse")]]))
...
async def next_page(bot, query):
    ...
    if settings['button']:
        btn = [[InlineKeyboardButton( text=f"[{get_size(file.file_size)}] {file.file_name.replace('@namaste cineverse','')}", callback_data=f'{pre}#{file.file_id}'),]for file in files]
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name.replace('@namaste cineverse','')}", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"⚜️[{get_size(file.file_size)}] {file.file_name.replace('@namaste cineverse','')}", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{file.file_name.replace('@namaste cineverse','')}",
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        #     except Exception as e:
        #         logger.exception(e)
        #     #TODO f_caption = f_caption REMOVED THIS LIKE
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/namaste_cineverse")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            return
    # don't change anything without contacting me @Namaste_CineVerse_Bot
        ...
        # [  InlineKeyboardButton('ꜰɪʟᴇ ꜱᴛᴏʀᴇ', callback_data='store_file'),#  InlineKeyboardButton('ᴛᴇʟᴇɢʀᴀᴘʜ', callback_data='tele') ],
        [ 
             InlineKeyboardButton('ᴄᴏɴɴᴇᴄᴛɪᴏɴꜱ', callback_data='coct')
            #  InlineKeyboardButton('ꜰɪʟᴛᴇʀꜱ', callback_data='filters')
         ],
        #TODO
        # [InlineKeyboardButton('ʏᴛ-ᴅʟ', callback_data='ytdl'), InlineKeyboardButton('ꜱʜᴀʀᴇ ᴛᴇxᴛ', callback_data='share')],
        [ InlineKeyboardButton('ꜱᴏɴɢ', callback_data='song')
        #  ,InlineKeyboardButton('ᴇᴀʀɴ ᴍᴏɴᴇʏ', callback_data='shortlink_info')
         ],
        # [InlineKeyboardButton('ꜱᴛɪᴄᴋᴇʀ-ɪᴅ', callback_data='sticker'),InlineKeyboardButton('ᴊ-ꜱᴏɴ', callback_data='json')],
        [             
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://t.me/namaste_cineverse")
        ],[
        ...
        auth_users_now = [804504706]  # Example list of authorized user IDs

        if query.from_user.id not in auth_users_now:
            print(query.from_user.id)
            return await query.answer("This is only for admins")
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/Namaste_CineVerse_Bot")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            ...
            for file in files:
                cap += f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {file.file_name.replace('@namaste cineverse','')}\n</a></b>"
    else:
        if settings["button"]:
            ...
            for file in files:
                cap += f"<b>📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {file.file_name.replace('@namaste cineverse','')}\n\n</a></b>"
