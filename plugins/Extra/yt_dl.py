...
async def song(client, message):
    ...
    try:
        ...
        # Fix filename construction and handle potential encoding issues
        short_title = title[:20].replace(" ", "_")
        encoded_title = short_title.encode('utf-8')
        thumb_name = f'thumb_{encoded_title.decode("ascii", errors="ignore")}.jpg'

        thumb = requests.get(thumbnail, allow_redirects=True)
        open(thumb_name, 'wb').write(thumb.content)

        performer = f"[NC NETWORKS™]"
    ...
    try:
        ...
        cap = "**BY›› [NC NETWORKS™](https://t.me/namaste_cineverse)**"
